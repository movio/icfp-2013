...
object Program extends Parser with App { tests()

  // hack around below here

  eval(
    0x1122334455667788L,
    "(lambda (x) (fold x 0 (lambda (y z) (if0 y y y))))"
  )
  ...
  // hack around above here


  def eval(input: Long, prog: String): Long = {
    println
    print("in:   ")
    output(input)
